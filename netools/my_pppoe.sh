...
#shell测试命令 输入crontab -e 复制计划任务其中一条 即可 如需修改保存地址 请看第75和76行
#./my_pppoe.sh runA

#计划任务命令
#*/5 * * * * /usr/share/my_pppoe.sh runA >> /usr/share/my_pppoe.log 2>&1
####
#1分钟执行一次，异常重启wan
#*/1 * * * * /usr/share/my_pppoe.sh runA >> /usr/share/my_pppoe.log 2>&1
#2分钟执行一次，异常重启wan
#*/2 * * * * /usr/share/my_pppoe.sh runA >> /usr/share/my_pppoe.log 2>&1
#5分钟执行一次，异常重启wan
#2-59/5 * * * * /usr/share/my_pppoe.sh runA >> /usr/share//my_pppoe.log 2>&1
#5分钟执行一次，异常重启网络连接
#*/5 * * * * /usr/share/my_pppoe.sh runB >> /usr/share/my_pppoe.log 2>&1
#5分钟执行一次，异常重启路由器
#*/5 * * * * /usr/share/my_pppoe.sh runC >> /usr/share/my_pppoe.log 2>&1
...
LogMax=512
LogFile="/usr/share/my_pppoe.log"
